...
uint8_t Usart8_TxBuffer[128];

void Usart8DmaPrintf(const char *format,...)
{
	uint16_t len;
	va_list args;	
	va_start(args,format);
	len = vsnprintf((char*)Usart8_TxBuffer,sizeof(Usart8_TxBuffer)+1,(char*)format,args);
	va_end(args);
	HAL_UART_Transmit_DMA(&huart8, Usart8_TxBuffer, len);
}
/* USER CODE END 0 */
